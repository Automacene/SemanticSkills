...
def convert_skill(directory, output_dir, base_dir):
    ...
    # Extract skill name and relative parent path
    skill_dir = os.path.basename(directory)
    skill_name = skill_dir.lower()
    rel_parent = os.path.relpath(os.path.dirname(directory), base_dir)
    ...
    if 'completion' in config:
        ...
        if 'stop_sequences' in completion and len(completion['stop_sequences']) > 0:
            skill_content += "    stop:\n"
            for stop_seq in completion['stop_sequences']:
                skill_content += f"      - \"{stop_seq}\"\n"

    # Write skill file to correct parent folder in output directory
    skill_filename = os.path.join(target_dir, f"{skill_name}.skill")
    ...
    try:
        ...
        print(f"Error writing {skill_filename}: {e}")
        return False
...
def search_directories(base_dir, output_dir):
    ...
    for root, dirs, files in os.walk(base_dir):
        # Move any .skill files found
        for file in files:
            if file.endswith('.skill'):
                rel_parent = os.path.relpath(root, base_dir)
                target_dir = os.path.join(output_dir, rel_parent)
                os.makedirs(target_dir, exist_ok=True)
...
def main():
    ...
    parser.add_argument("-i", "--input", required=True,
                        help="Base directory to search for skills (e.g., ./Skills)")
    parser.add_argument("-o", "--output", required=True,
                        help="Output directory for .skill files (e.g., ./ConvertedSkills)")
    args = parser.parse_args()

    # Get absolute paths
    base_dir = os.path.abspath(args.input)
    # Output directory should be the same as input if you want .skill files in the parent folder
    ...
    # Place .skill files directly in output_dir, not in a subfolder
